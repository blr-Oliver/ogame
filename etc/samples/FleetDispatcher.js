...
FleetDispatcher.prototype.refreshAPIData = function () {
  ...
  let apiData = [].concat(this.apiCommonData).concat(this.apiTechData).concat(apiShipData).concat(this.apiDefenseData).map(function (item) {
    return item.join(';');
  }).join('|');
  let title = this.loca.LOCA_API_FLEET_DATA + '<br/><input id="FLEETAPI" value="' + apiData + '" readonly onclick="select()" style="width: 360px;"></input>';
...
FleetDispatcher.prototype.refreshNavigationFleet2 = function (displayErrors = false) {
  ...
  if (!this.hasShipsSelected()) {
    ...
    $('#sendFleet').attr('class', 'start on').removeAttr('disabled');
  }
...
FleetHelper.prototype.getFleetSpeedFaktor = function (mission) {
  ...
  if (peaceful.includes(mission))
    return this.SPEEDFAKTOR_FLEET_PEACEFUL;
  else if (war.includes(mission))
    return this.SPEEDFAKTOR_FLEET_WAR;
  else if (holding.includes(mission))
    return this.SPEEDFAKTOR_FLEET_HOLDING;
  else return 0;
};
